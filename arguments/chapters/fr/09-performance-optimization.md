...
## 1. Le comportement de rendu de React

### Le processus de rendu

Le processus de rendu de React se compose de deux phases principales : le **render** (création du DOM virtuel) et le **commit** (mise à jour du DOM réel).
...
### Quand les composants se re-rendent
...
### Phase de render vs phase de commit
...
## 2. React.memo : empêcher les re-renders
...
### React.memo avec des props complexes
...
### Flux de décision de React.memo

Lorsque le parent se re-rend, React parcourt cette décision pour déterminer si un enfant memoïsé doit lui aussi se rendre. Le contrôle d'égalité superficielle (shallow-equal) est le point critique — les objets/fonctions inline le mettent en échec.
...
## 3. Patterns useMemo et useCallback

### useMemo : memoïser des valeurs
...
### useCallback : memoïser des fonctions
...
Tous deux mettent en cache quelque chose entre les renders, mais ils ne cachent pas la même chose. Utilisez cet arbre pour choisir le bon outil — et n'oubliez pas que la bonne réponse est souvent « ni l'un ni l'autre ».
...
### Exemple concret : un dashboard optimisé
...
## 5. Optimisation de la taille du bundle
...
### Imports dynamiques pour les grandes bibliothèques
...
## 6. Virtual scrolling pour les grandes listes

### Comment fonctionne la virtualisation

Au lieu de rendre les 10 000 lignes d'emblée, un virtualizer mesure la zone d'affichage (viewport), calcule la portion de la liste qui est visible (plus une petite marge de buffer pour un défilement fluide) et ne monte que ces lignes. Le reste est représenté par un unique espaceur de grande hauteur.
...
### React Window (utilisation de base)
...
### Infinite scroll avec React Window
...
## 7. Profiling avec React DevTools
...
### Comprendre les métriques du Profiler
...
### Hook de suivi des performances personnalisé
...
## 10. Choix de la stratégie d'optimisation
...
## Conclusion : la maîtrise des performances

**Ottimizzazione magistrale! ⚡** Profilez toujours d'abord, optimisez ensuite. Concentrez-vous sur la performance perçue par l'utilisateur et fixez des budgets réalistes.
