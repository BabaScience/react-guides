...
## 1. Paradigmes de gestion d'état

### Hiérarchie de classification de l'état
...
## 2. État local vs global : cadre de décision

### Principe de colocalisation de l'état

**L'état doit vivre aussi près que possible de l'endroit où il est utilisé.** N'élevez l'état au niveau global que lorsque c'est réellement nécessaire, à travers des composants déconnectés.
...
## 3. Context API : état global simple

### Arbre de providers et portée des consumers

Chaque `Provider` injecte une valeur dans le sous-arbre situé en dessous de lui. Les consumers remontent l'arbre jusqu'au provider correspondant le plus proche — l'ordre de composition importe, car les providers internes peuvent surcharger les providers externes.
...
### Implémentation de base d'un Context
...
### Composition de plusieurs Contexts
...
### Optimisation des performances du Context
...
## 4. Redux Toolkit : le Redux moderne
...
### Configuration de base du store
...
### Utilisation de Redux dans les composants
...
### Exemple de slice complexe : liste de tâches
...
## 5. Plongée approfondie dans les concepts fondamentaux de Redux
...
## 6. Middleware et opérations asynchrones

### Cycle de vie de bout en bout d'un async thunk

`createAsyncThunk` dispatch automatiquement les actions `pending`, `fulfilled` ou `rejected` autour de votre fonction asynchrone. Le middleware se situe entre le dispatch et le reducer, c'est là que la fonction thunk est réellement exécutée au lieu d'être transmise comme une simple action.
...
### Exemple asynchrone complexe : opérations CRUD
...
### RTK Query (récupération de données avancée)
...
## 7. Zustand : gestion d'état minimaliste

### Comment un store extérieur à React re-render les composants

Zustand conserve l'état dans un store JavaScript simple qui vit en dehors de l'arbre React. Les composants s'abonnent via un selector ; lorsque le store change, seuls les composants dont la portion sélectionnée a changé sont re-rendus.
...
### Zustand avec actions asynchrones
...
## 8. Jotai : gestion d'état atomique
...
## 9. Recoil : état basé sur un graphe
...
## 10. Matrice de sélection pour la gestion d'état
...
## Conclusion : maîtriser la gestion d'état
