...
## 1. Paradigmi di gestione dello stato

### Gerarchia di classificazione dello stato
...
### Confronto tra gli approcci di gestione dello stato
...
## 2. Stato locale vs globale: framework decisionale

### Principio di colocazione dello stato

**Lo stato dovrebbe vivere il più vicino possibile al punto in cui viene utilizzato.** Eleva lo stato all'ambito globale solo quando è realmente necessario condividerlo tra componenti scollegati.
...
### Pattern di sollevamento dello stato
...
## 3. Context API: stato globale semplice

### Albero dei provider e portata dei consumer

Ogni `Provider` inietta un valore nel sottoalbero sottostante. I consumer risalgono l'albero fino al provider corrispondente più vicino: l'ordine di composizione è importante perché i provider interni possono sovrascrivere quelli esterni.
...
### Implementazione base del Context
...
### Ottimizzazione delle performance del Context
...
### Configurazione base dello store
...
### Utilizzo di Redux nei componenti
...
### Esempio di slice complesso: lista di task
...
## 5. Approfondimento sui concetti fondamentali di Redux
...
## 6. Middleware e operazioni asincrone

### Ciclo di vita di un thunk asincrono dall'inizio alla fine

`createAsyncThunk` invia automaticamente le action `pending`, `fulfilled` o `rejected` attorno alla tua funzione asincrona. Il middleware si colloca tra il dispatch e il reducer, ed è proprio lì che la funzione thunk viene effettivamente eseguita invece di essere inoltrata come una semplice action.
...
### Esempio asincrono complesso: operazioni CRUD
...
### RTK Query (recupero dati avanzato)
...
## 7. Zustand: gestione dello stato minimalista

### Come uno store esterno a React fa il re-render dei componenti

Zustand mantiene lo stato in un semplice store JavaScript che vive al di fuori dell'albero di React. I componenti si sottoscrivono tramite un selector; quando lo store cambia, vengono ri-renderizzati solo i componenti la cui porzione selezionata è cambiata.
...
### Pattern degli slice in Zustand
...
## 8. Jotai: gestione atomica dello stato
...
## 9. Recoil: stato basato su grafo
...
## 10. Matrice di selezione per la gestione dello stato
...
## Conclusione: padronanza della gestione dello stato
