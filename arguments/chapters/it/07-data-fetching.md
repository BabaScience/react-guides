...
1. [Paradigmi di Data Fetching](#1-data-fetching-paradigms)
2. [Fetch API Nativa e Axios](#2-native-fetch-api-and-axios)
3. [Stati di Caricamento e Gestione degli Errori](#3-loading-states-and-error-handling)
4. [React Query: Gestione del Server State](#4-react-query-server-state-management)
5. [SWR: Stale-While-Revalidate](#5-swr-stale-while-revalidate)
6. [Aggiornamenti Ottimistici](#6-optimistic-updates)
7. [Strategie di Gestione della Cache](#7-cache-management-strategies)
8. [Polling e Aggiornamenti in Tempo Reale](#8-polling-and-real-time-updates)
9. [Pattern Avanzati](#9-advanced-patterns)
10. [Selezione della Strategia di Data Fetching](#10-data-fetching-strategy-selection)

---

## 1. Paradigmi di Data Fetching

### Server State vs Client State

Il **server state** è fondamentalmente diverso dal client state: è asincrono, potenzialmente obsoleto ed esiste all'esterno. Le soluzioni tradizionali di gestione dello state mescolano queste preoccupazioni.
...
## 3. Stati di Caricamento e Gestione degli Errori

### Il Ciclo di Vita della Richiesta come Macchina a Stati

Modellare una richiesta fetch come una macchina a stati finiti rende espliciti i quattro stati mutuamente esclusivi e previene UI impossibili come "loading + error mostrati contemporaneamente".
...
### Pattern di UI per gli Stati di Caricamento
...
## 4. React Query: Gestione del Server State
...
### useMutation per gli Aggiornamenti
...
## 6. Aggiornamenti Ottimistici

### Flusso di un Aggiornamento Ottimistico

La UI applica la modifica localmente prima che la rete risponda, per poi confermarla con la risposta del server o effettuare un rollback in caso di fallimento. È questo che rende le interazioni istantanee.
...
### Aggiornamenti Ottimistici con React Query
...
### Esempio di Aggiornamento Ottimistico Complesso
...
## 7. Strategie di Gestione della Cache

### La Decisione di Cache Hit/Miss

Ogni query consulta prima la cache tramite la propria key. Un hit fresco restituisce i dati istantaneamente con zero costo di rete; un miss (o una voce obsoleta) innesca un fetch reale e popola la cache per la volta successiva.
...
### Configurazione della Cache di React Query
...
### Manipolazione Manuale della Cache
...
## 8. Polling e Aggiornamenti in Tempo Reale

### Il Ciclo di Polling

Il polling è un loop: fetch, aggiornamento della UI, attesa, ripetizione — finché un segnale di stop (unmount, condizione di successo o perdita del focus della tab) interrompe il ciclo.
...
### Aggiornamenti in Tempo Reale Ottimistici
...
## 10. Selezione della Strategia di Data Fetching
...
## Conclusione: Padroneggiare il Data Fetching
